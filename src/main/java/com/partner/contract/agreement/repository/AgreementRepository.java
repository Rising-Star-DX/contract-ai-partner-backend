...
public interface AgreementRepository extends JpaRepository<Agreement, Long> {
    @Query("select a from Agreement a join a.category c order by a.createdAt desc")
    List<Agreement> findAllWithCategoryByOrderByCreatedAtDesc();
    @Query("select a from Agreement a join a.category c where a.name like %:name%")
    List<Agreement> findWithCategoryByNameContaining(@Param("name") String name);
    @Query("select a from Agreement a join a.category c where a.category.id = :categoryId")
